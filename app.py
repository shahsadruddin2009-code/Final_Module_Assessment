...
def search():
    ...
    Supports query parameter 'query' for search terms.
    Returns filtered books or all books if no query provided.
    """
    query = request.args.get('query', '').strip()
    current_user = get_current_user()
    
    if not query:
        # If no query provided, return all books with search results message
        return render_template('index.html', 
                             books=BOOKS, 
                             cart=cart, 
                             current_user=current_user, 
                             search_query=query,
                             search_message="Search results - Please enter a search term")
    
    # Search for books matching the query (case-insensitive)
    matching_books = []
    query_lower = query.lower()
    
    for book in BOOKS:
        # Search in title, category, and author (if available)
        if (query_lower in book.title.lower() or 
            query_lower in book.category.lower()):
            matching_books.append(book)
    
    if matching_books:
        search_message = f"Found {len(matching_books)} result(s) for '{query}'"
    else:
        search_message = f"No results found for '{query}'"
...
def register():
    ...
    if request.method == 'POST':
        ...
        password = request.form.get('password', '')  # Don't escape password
